...
class Hostcms_Bitrix24_Controller extends Core_Servant_Properties
{
	 ...
	 * @param Shop_Order_Model $oShop_Order shop order object
	 * @return int|NULL
	 * @hostcms-event Hostcms_Bitrix24_Controller.onBeforeCompanyAdd
	 * @hostcms-event Hostcms_Bitrix24_Controller.onBeforeRequisiteAdd
	 */
	protected function _getCompanyId(Shop_Order_Model $oShop_Order)
	{
		...
		else
		{
			...
			Core_Event::notify('Hostcms_Bitrix24_Controller.onBeforeCompanyAdd', $this, array($queryCompanyData));

			$aResult = $this->_sendData('crm.company.add', $queryCompanyData);

			if (isset($aResult['result']))
			{
				$queryRequisiteData = array(
					'fields' => array(
						'ENTITY_TYPE_ID' => 4, // 4 - is company in CRest::call('crm.enum.ownertype');
						'ENTITY_ID' => $aResult['result'],//company id
						'PRESET_ID' => 1, // 1 - Организация, 3 - ИП, 5 - Физ.лицо
						'TITLE' => $oShop_Order->company,
						'ACTIVE' => 'Y',
						'NAME' => $oShop_Order->company,
						'RQ_INN' => $oShop_Order->tin,
						'RQ_KPP' => $oShop_Order->kpp,
					)
				);

				Core_Event::notify('Hostcms_Bitrix24_Controller.onBeforeRequisiteAdd', $this, array($queryRequisiteData));

				$aResult = $this->_sendData('crm.requisite.add', $queryRequisiteData);

				if (isset($aResult['error']))
				{
					Core_Log::instance()->clear()
						->status(Core_Log::$ERROR)
						->write('getCompanyId(crm.requisite.add) error: ' . $aResult['error_description']);
				}
	 ...
	 * @param Shop_Order_Model $oShop_Order shop order object
	 * @return int|NULL
	 * @hostcms-event Hostcms_Bitrix24_Controller.onBeforeContactAdd
	 */
	protected function _getContactId(Shop_Order_Model $oShop_Order)
	{
		...
		else
		{
			...
			Core_Event::notify('Hostcms_Bitrix24_Controller.onBeforeContactAdd', $this, array($queryContactData));
	 ...
	 * @hostcms-event Hostcms_Bitrix24_Controller.onBeforeLeadAdd
	 */
	public function formFill(Form_Fill_Model $oForm_Fill)
	{
		if (!is_null($oForm_Fill) && $this->_config['form'])
		{
			...
			Core_Event::notify('Hostcms_Bitrix24_Controller.onBeforeLeadAdd', $this, array($oForm_Fill, $queryData));
